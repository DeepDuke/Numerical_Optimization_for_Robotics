...
class CubicSpline
{
   public:
    ...
    void Update(const Eigen::Matrix2Xd& inner_points)
    {
        ...
        UpdateMatrixB(inner_points);

        UpdateCubicCurve(inner_points);
    }
    ...
    double GetStretchEnergy() const
    {
        ...
        double energy = 0.0;
        for(size_t i = 0; i < cubic_curve_.getPieceNum(); ++i)
        {
   ...
   private:
    ...
    {
        if (inner_points.cols() != inner_points_num_)
        {
    ...
    void UpdateMatrixB(const Eigen::Matrix2Xd& inner_points)
    {
        ...
        for (size_t i = 0; i < inner_points_num_; ++i)
        {
            ...
            {
                // D1 = 3*(x2 - x0)
                b_(i, 0) = 3 * (inner_points(0, 1) - 0);
                b_(i, 1) = 3 * (inner_points(1, 1) - 0);
            }
            else if (i < inner_points_num_ - 1)
            {
                ...
                size_t idx1 = i - 1;
                size_t idx2 = (i + 2) - 1;
                auto col1 = inner_points.col(idx1);
                auto col2 = inner_points.col(idx2);
            ...
            else
            {
                ...
                auto col = inner_points.col(i - 1);  // x_{n-2}
                b_(i, 0) = 3 * (0 - col(0));
                b_(i, 1) = 3 * (0 - col(1));
            }
        }
    ...
    void UpdateCubicCurve(const Eigen::Matrix2Xd& inner_points)
    {
        ...
        for (size_t i = 1; i < pieces_num_ - 1; ++i)
        {
            ...
            auto xi_next = inner_points.col(i);
            auto Di = b_.row(i - 1);
        ...
        // x_{n-1}
        auto prev_xn = inner_points.col(inner_points_num_ - 1);
        // D_{n-1}
   ...
   private:
    ...
    Eigen::Matrix2Xd inner_points_gradients_;
    Eigen::MatrixXd A_;
