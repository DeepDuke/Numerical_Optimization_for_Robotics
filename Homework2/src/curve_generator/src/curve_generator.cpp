...
struct Config
{
    ...
    Config(const ros::NodeHandle& nh_priv)
    {
        ...
        circleObs = Eigen::Map<const Eigen::Matrix<double, 3, -1, Eigen::ColMajor>>(circleObsVec.data(), 3,
                                                                                    circleObsVec.size() / 3);
    }
};
...
class CurveGen
{
   ...
   public:
    CurveGen(ros::NodeHandle& nh_) : config(ros::NodeHandle("~")), nh(nh_), visualizer(nh)
    {
        targetSub =
            nh.subscribe(config.targetTopic, 1, &CurveGen::targetCallBack, this, ros::TransportHints().tcpNoDelay());
    }
    ...
    inline void plan()
    {
        if (startGoal.size() == 2)
        {
            ...
            path_smoother::PathSmoother pathSmoother(startGoal.front(), startGoal.back(), N, config.circleObs, config.penaltyWeight);
            CubicCurve curve;
